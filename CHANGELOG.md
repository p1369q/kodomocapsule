...
* ログイン前トップのCTA文言を改善
* 「まずは登録なしで体験する」を目立たせた
* Googleログイン導線を「無料で保存をはじめる」に変更
* 「Googleアカウントでログインします」の補足を追加
* 「無料で10カプセルまで保存できます」「勝手に課金されることはありません」の安心文言を追加
* 共有範囲に関する安心文言を追加
...
* スマホ下部ナビから「家族」を外した
* 下部ナビを「ギャラリー / 写真一覧 / ＋追加 / 設定」中心に整理
* 写真一覧の「表示中の写真をまとめてDL」ボタンを非表示/削除
* 設定ドロワー上部の見出し・説明文をiPhone幅で読みやすく調整
* v316のスマホ下部ナビ表示条件とセーフエリア対応は維持
* Firebase / Firestore / Storage / Stripe / Cloud Run / 家族共有 / 保存処理は変更なし
* Minecraft生成ロジックは変更なし
* JavaScript構文チェック済み
